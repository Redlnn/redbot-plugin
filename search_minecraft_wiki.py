...
async def group_message_listener(app: GraiaMiraiApplication, group: Group, message: MessageChain):
    ...
    if cmd[0] not in ('!', '！'):
        return 0
    args: tuple = cmd[1:].strip().split(' ', 1)
    if len(args) < 2:
        return 0
    ...
    msg_id1 = await app.sendGroupMessage(group, MessageChain.create([App(content=json.dumps(bilibili_wiki_json), type='App')]))
    sleep(round(uniform(0.3, 0.9), 2))
    msg_id2 = await app.sendGroupMessage(group, MessageChain.create([App(content=json.dumps(fandom_gamepedia_wiki_json), type='App')]))
    ...
    if msg_id1.messageId <= 0 or msg_id2.messageId <= 0:
        ...
        await app.sendGroupMessage(group, MessageChain.create([Plain(msg_send)]), quote=message.get(Source).pop(0))  # noqa
